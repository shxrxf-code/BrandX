...
const footerLinks: Record<string, { href: string; label: string; external?: boolean }[]> = {
  ...
  Connect: [
    ...
    { href: 'https://github.com/brandex', label: 'GitHub', external: true },
  ],
}
  ...
  return (
    <footer className="relative bg-subtle border-t border-border overflow-hidden">
      ...
      <div className="max-w-content mx-auto px-6 md:px-10 py-20 md:py-28">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-16 lg:gap-24">
          <div className="lg:col-span-1">
            ...
            <p className="text-sm text-muted mt-4 leading-relaxed max-w-xs">
              A digital experience studio engineering premium products that drive business growth.
            </p>
            <div className="mt-8 flex items-center gap-4">
              {['TW', 'LI', 'IG', 'GH'].map((social) => (
                <Link
                  key={social}
                  href="#"
                  className="w-10 h-10 rounded-full border border-border-light flex items-center justify-center text-[10px] text-muted hover:text-foreground hover:border-accent hover:bg-accent/10 transition-all duration-400"
                >
                  {social}
                </Link>
              ))}
                ...
                {links.map((link) => (
                  <li key={link.label}>
                    ...
                    >
                      {link.label}
                      {link.external && (
                        <span className="text-[10px] text-muted group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-300">↗</span>
                      )}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-20 md:mt-28 pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted">&copy; {new Date().getFullYear()} Brandex Digital. All rights reserved.</p>
          <div className="flex items-center gap-6">
            <Link href="/privacy" className="text-xs text-muted hover:text-foreground transition-colors duration-300">Privacy</Link>
            <Link href="/terms" className="text-xs text-muted hover:text-foreground transition-colors duration-300">Terms</Link>
          </div>
        </div>
      </div>
    </footer>
