...
const words = ['Design', 'Technology', 'Growth']
...
export default function Preloader({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const timeline = async () => {
      await new Promise(r => setTimeout(r, logoDuration))
      setStage('words')

      for (let i = 0; i < words.length; i++) {
        setCurrentWord(i)
        const pct = ((i + 1) / words.length) * 70
        setProgress(pct)
      ...
      setStage('welcome')
      setProgress(90)
      await new Promise(r => setTimeout(r, welcomeDuration))
        ...
        >
          <div className="relative flex flex-col items-center gap-8">
            ...
            {stage === 'welcome' && (
              ...
              >
                <motion.span
                  className="text-xl font-display font-light tracking-[0.4em] uppercase text-foreground/80 block mb-3"
                >
                  Welcome to
                </motion.span>
                <span className="text-3xl font-display font-bold tracking-tight text-foreground">
